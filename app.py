...
if uploaded_file:
    ...
    def preprocess_data(df):
        ...
        global label_encoders  # Make it accessible globally
        label_encoders = {}

        for feature in categorical_features:
            le = LabelEncoder()
            df[feature] = le.fit_transform(df[feature])
            label_encoders[feature] = le  # Store encoder for later use
    ...
    with st.spinner("Training in progress... ⏳"):
        history = model.fit(
            X_train,
            {'math_output': y_train['math_score'],
             'reading_output': y_train['reading_score'],
             'writing_output': y_train['writing_score']},
            validation_data=(
                X_test,
                {'math_output': y_test['math_score'],
                 'reading_output': y_test['reading_score'],
                 'writing_output': y_test['writing_score']}
            ),
            epochs=50,
            batch_size=32,
            verbose=0
    ...
    axes[0].scatter(y_test['math_score'], math_predictions, alpha=0.5, color='b')
    axes[0].set_xlabel("Actual Math Score")
    axes[0].set_ylabel("Predicted Math Score")
    axes[0].set_title("Math Score Predictions")

    axes[1].scatter(y_test['reading_score'], reading_predictions, alpha=0.5, color='g')
    ...
    axes[2].scatter(y_test['writing_score'], writing_predictions, alpha=0.5, color='r')
        ...
        for feature in input_data.columns:
            if feature in label_encoders:
                try:
                    ...
                    input_data[feature] = 0  # Assign a default value for unseen labels
